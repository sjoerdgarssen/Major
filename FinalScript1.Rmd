...
In this document all the figures of the thesis and the SI are made. 
The selfwritten functions are written in the other script document. 
Thus, the other script needs to be fully runned before executing this script. 
Note that some chunks can be used to make multiple models when running it two times with small adaptions in the script. 
When this yields for a chunk, then this is always indicated in the comments of that chunk. 
...
```{r}
#In this Chunk the Model Selection plot is made. This is figure 2 in the paper.
#-------------
...
dfRMSEP=data.frame(ss=1:100,RMSECV=NA,DQ2=NA,NMC=NA,BTS=NA, BTSacc=NA)
dfACC=data.frame(ss=1:100,RMSECV=NA,DQ2=NA,NMC=NA,BTS=NA, BTSacc=NA)
dfSn=data.frame(ss=1:100,RMSECV=NA,DQ2=NA,NMC=NA,BTS=NA, BTSacc=NA)
dfSp=data.frame(ss=1:100,RMSECV=NA,DQ2=NA,NMC=NA,BTS=NA, BTSacc=NA)
  ...
  for (j in c('RMSECV','DQ2','NMC','BTS','BTSacc')){
    ...
    if (j=='RMSECV' || j=='DQ2' || j=='NMC'){
      ...
      for (k in c(25)){ #this could be extended to multiple values for k in this way.
        ...
        #next lines: calculate RMSECV, Accuracy, Sensitivity and Specificity with their respective self-written functions and store them in their respective data frames.
        dfRMSEP[i,index]=RMSE(pred=pred,c=c)
        dfACC[i,index]=acc(c=c,pred=pred,threshold=0)
        dfSn[i,index]=sensitivity(pred=pred,c=c,threshold=0)
        dfSp[i,index]=specificity(pred=pred,c=c,threshold=0)
        index=index+1
      }}
    ...
    #BTS 0.632 error.Both RMSE and Accuracy as error metric.
      ...
      #next lines: calculate and store measurements
      dfRMSEP[i,index]=RMSE(pred=pred,c=c)
      dfACC[i,index]=acc(c=c,pred=pred,threshold=0)
      dfSn[i,index]=sensitivity(pred=pred,c=c,threshold=0)
      dfSp[i,index]=specificity(pred=pred,c=c,threshold=0)
...
dfRMSEP_m=dfRMSEP[,-1]%>%apply(2,mean)
dfRMSEP_s=dfRMSEP[,-1]%>%apply(2,sd)
dfACC_m=dfACC[,-1]%>%apply(2,mean)
dfACC_s=dfACC[,-1]%>%apply(2,sd)
dfSn_m=dfSn[,-1]%>%apply(2,mean)
dfSn_s=dfSn[,-1]%>%apply(2,sd)
dfSp_m=dfSp[,-1]%>%apply(2,mean)
dfSp_s=dfSp[,-1]%>%apply(2,sd)
...
#make method vs accuracy plot
plotacc=data.frame(method=c('RMSECV','DQ2','NMC','BTS','BTS ACC','Reference'),colour=c(rep('1CV',3),'Bootstrap','Bootstrap','Other'),acc=c(dfACC_m,rdcv$ACC_mean),sd=c(dfACC_s,rdcv$ACC_sd))%>%ggplot(aes(x=reorder(method,acc),y=acc,ymin=acc-sd,ymax=acc+sd,col=colour))+geom_point()+geom_errorbar()+labs(y='Accuracy (%)',x='Metric',col='Method')

#make method vs RMSEP plot
plotrmsecv=data.frame(method=c('RMSECV','DQ2','NMC','BTS','BTS ACC','Reference'),colour=c(rep('1CV',3),'Bootstrap','Bootstrap','Other'),acc=c(dfRMSEP_m,rdcv$RMSEP_mean),sd=c(dfRMSEP_s,rdcv$RMSEP_sd))%>%ggplot(aes(x=method,y=acc,ymin=acc-sd,ymax=acc+sd,col=colour))+geom_point()+geom_errorbar()+labs(y='RMSEP')+labs(title='100 small sets. Performance determined on large set.',x='',y='RMSEP',col='Method')

#make method vs sensitivity plot
plotsn=data.frame(method=c('RMSECV','DQ2','NMC','BTS','BTS ACC','Reference'),colour=c(rep('1CV',3),'Bootstrap','Bootstrap','Other'),acc=c(dfSn_m,rdcv$SN_mean),sd=c(dfSn_s,rdcv$SN_sd))%>%ggplot(aes(x=method,y=acc,ymin=acc-sd,ymax=acc+sd,col=colour))+geom_point()+geom_errorbar()+labs(title='100 small sets. Performance determined on large set.',x='',y='Sensitivity',col='Method')

#make method vs specifity plot
plotsp=data.frame(method=c('RMSECV','DQ2','NMC','BTS','BTS ACC','Reference'),colour=c(rep('1CV',3),'Bootstrap','Bootstrap','Other'),acc=c(dfSp_m,rdcv$SP_mean),sd=c(dfSp_s,rdcv$SP_sd))%>%ggplot(aes(x=method,y=acc,ymin=acc-sd,ymax=acc+sd,col=colour))+geom_point()+geom_errorbar()+labs(title='100 small sets. Performance determined on large set.',x='',y='Specificity',col='Method')

#make one plot of the 4 above mentioned plots together.
plot=ggarrange(plotacc,plotrmsecv,plotsn,plotsp)+labs(title='20 sets')

```

```{r}
#in this and the next Chunks, the predictive performance plot (figure 4 in thesis) is made. 
#This chunk needs to be run two times. First run this chunk, then make the small adaptions seen in the comments, and then run this chunk again. Hereafter, the next chunk makes the figure.  
#--------
...
for (i in 1:50){ #repeat whole process for 50 small sets (N=50).
  ...
  pp[i,2]=acc(c=smallc,pred=pred) #store accuracy of predicition.
  ...
  Aopt=LV_select_r(X=X_,c=c_,A=25,r=50,method='BTSacc') #Select Aopt of the 788 samples based on bootstrap accuracy with 50 repeats. 25 components is the max Aopt considered.
  model=plsda_function(X=X_,c=c_,A=Aopt) #make model on remainder
  pred=prediction(X=smallx,m=model) #predict small set
  pp[i,9]=acc(c=smallc,pred=pred) #store the accuracy of this prediction as reference (9th column). THis column is later needed for other figures too. 
...
ppr=data.frame(method=c('FIT','5-fold CV','25-fold CV','LOOCV','25-fold DV','5-fold DV','BTS','Reference'),acc=pp_random[,-1]%>%apply(2,mean),sd=pp_random[,-1]%>%apply(2,sd)) #get the results from random population in the right structure to make plot.
plot=data.frame(method=c('FIT','5-fold CV','25-fold CV','LOOCV','25-fold DV','5-fold DV','BTS','Reference'),acc=pp_true[,-1]%>%apply(2,mean),sd=pp_true[,-1]%>%apply(2,sd))%>%ggplot(aes(x=reorder(method,acc),y=acc,ymin=acc-sd,ymax=acc+sd,col='True label'))+geom_errorbar()+geom_point()+labs(x='Method',y='Accuracy (%)',col='')+geom_errorbar(data=pp_random,aes(x=method,y=acc,ymin=acc-sd,ymax=acc+sd,col='Random label'))+geom_point(data=pp_random,aes(x=method,y=acc,ymin=acc-sd,ymax=acc+sd,col='Random label'))
```

```{r}
#In this chunk the threshold plots are made (figure 2 in thesis)
...
for (i in 1:20){ #repeat process for 20 small sets.
  #make small set
  small=smallsample(X=X,c=c,n=50,ss=i,bal=4) #Change bal to 1 for balanced classes.
  smallx=small[['small']][['Xsmall']]
  smallc=small[['small']][['csmall']]
...
#make data frame used to make figure 2
...
plot<-undfsum%>%ggplot(aes(x=metric,y=mean,ymin=mean-sd,ymax=mean+sd,col=threshold))+geom_point()+geom_errorbar()+labs(y='Value (%)',col='Threshold Method',x='Metric') #change plot name when repeating this chunk for class balance, otherwise the unbalanced classes plot will be overwritten.

```

```{r}
#in this chunk figure 1 (thesis) is made.
#-----
...
store=data.frame(ss=1:50,n10=NA,n20=NA,n30=NA,n40=NA,n50=NA,n70=NA,n100=NA,n200=NA)
for (i in 1:50){
  index=2
...
#make figure
plot=data.frame(size=c(10,20,30,40,50,70,100,200),acc=store[,-1]%>%apply(2,mean),sd=store[,-1]%>%apply(2,sd))%>%ggplot(aes(x=size,y=acc,ymin=acc-sd,ymax=acc+sd))+geom_line()+geom_errorbar()+labs(x='#samples in training set',y='Accuracy (%)')

```

```{r}
#in this chunk figure 6 of the thesis and SI figure 5 can be made.
...
for (i in 1:50){
  small=smallsample(X=X,c=c,n=50,ss=i,bal=1) #change to n=100 to make the SI figure 5
  smallx=small[['small']][['Xsmall']]
  smallc=small[['small']][['csmall']]
  for (r in 1:100){
...
srimp=data.frame(ss=rep(1:50,each=2),pos=rep(c('TP','FP'),50),randomCI=NA,doubleCI=NA,highest=NA) #store TPs etc. based on SR.
...
a=as.data.frame(vipimp%>%pivot_longer(c('randomCI','doubleCI','highest'),names_to='method',values_to='amount')%>%group_by(method,pos)%>%summarise(sum=sum(amount))) #get the right data frame structure to make the figure
b=data.frame(method=c('doubleCI','highest','randomCI'),pos=rep('FN',3),sum=150-(vipimp%>%pivot_longer(c('randomCI','doubleCI','highest'),names_to='method',values_to='amount')%>%group_by(method,pos)%>%summarise(sum=sum(amount))%>%filter(pos=='TP')%>%ungroup()%>%select(-c('method','pos')))) #calculate the False Negatives
a=a%>%rbind(b) #store the FNs also in the data frame
plotvip=a%>%ggplot(aes(x=method,y=sum,fill=pos))+geom_bar(stat='identity',position=position_dodge())+labs(fill='',y='#Variables',x='Method')+scale_x_discrete(labels = c('double CI','Highest 3','random CI'))+
  geom_text(aes(label=sum), vjust=0, color="black",
            position = position_dodge(0.9), size=3.5) #make plot based VIP. When repeating this chunk for the second time, than change the name of the plot, otherwise it will be overwritten

#below: exactly the same as above, but then based on SR.
a=as.data.frame(srimp%>%pivot_longer(c('randomCI','doubleCI','highest'),names_to='method',values_to='amount')%>%group_by(method,pos)%>%summarise(sum=sum(amount)))
b=data.frame(method=c('doubleCI','highest','randomCI'),pos=rep('FN',3),sum=150-(srimp%>%pivot_longer(c('randomCI','doubleCI','highest'),names_to='method',values_to='amount')%>%group_by(method,pos)%>%summarise(sum=sum(amount))%>%filter(pos=='TP')%>%ungroup()%>%select(-c('method','pos'))))
a=a%>%rbind(b)
plotsr=a%>%ggplot(aes(x=method,y=sum,fill=pos))+geom_bar(stat='identity',position=position_dodge())+labs(fill='',y='#Variables',x='Method')+scale_x_discrete(labels = c('double CI','Highest 3','random CI'))+
  geom_text(aes(label=sum), vjust=0, color="black",
            position = position_dodge(0.9), size=3.5) #make plot based on SR. When repeating this chunk for the second time, than change the name of the plot, otherwise it will be overwritten

#store the Aopt's of random labels needed to make other figures later in this markdown (figure 5 and SI figure 3):
...
```{r}
#in this Chunk figure 3b (thesis) is made
#-------
...
data.frame(method=colnames(Aopti[,-c(1,6)]),col=c(rep('1CV',3),'Bootstrap','Bootstrap'),mean=mean,sd=sd)%>%ggplot(aes(x=reorder(method,mean),y=mean,ymin=mean-sd,ymax=mean+sd,col=col))+geom_errorbar()+geom_point()+labs(x='Metric',y='Optimal number of components',col='Method') #get plot
```

```{r}
#in this chunk figure 7 is made.
#------
...
sc%>%ggplot(aes(x=V1,y=V2,col=as.factor(V3)))+geom_point()+labs(x='Scores component 1',y='Scores component 2',col='Class') #make score plot.
...
ggplot(pred,aes(x=pred,fill=as.factor(c)))+geom_density(alpha=0.5)+labs(fill='True Class:',x='Prediction',y='Density')+xlim(-5,5) #make distribution plot.
```

```{r}
#in this chunk the SI figure 6 is made.
...
#Supl Info: distribution plots for a 50- and a 100-sample set
#50 sample set:
...
for (r in 1:5){
  pred=pred%>%rbind(CVpredclass(X=smallx,c=smallc,Aopt=Aopt,ss=r,k=25))
}
...
```{r} 
#in this Chunk SI figure 1 is made.
#------

small=smallsample(X=X,c=c,n=50,ss=10,bal=1)
smallx=small[['small']][['Xsmall']]
smallc=small[['small']][['csmall']]
a=r1cvperformance(X=smallx,c=smallc,k=5,A=15,r=20,method='RMSECV') #Get the RMSECV mean and sd for every considered number of components. 
a$df%>%ggplot(aes(x=A,y=mean,ymin=mean-sd,ymax=mean+sd))+geom_errorbar()+geom_line()+geom_point()+labs(x='#Components',y='RMSECV') #figure A

a=BTS_LV_select(X=smallx,c=smallc,r=500,A=15) #get the RMSE values from every repeat and every number of components till 15 based on the bootstrap
means=a%>%apply(2,mean)
sds=a%>%apply(2,sd)
data.frame(A=1:15,means=means,sds=sds)%>%ggplot(aes(x=A,y=means,ymin=means-sds,ymax=means+sds))+geom_errorbar()+geom_line()+labs(x='#Components',y='0.632 Bootstrap RMSE')+geom_point() #figure B

```

```{r}
#This Chunk makes figure 5a.
#------
...
for (i in 1:10){ #repeat process for 10 small sets
  ...
  #get true predictive performances
  ...
  for (r in 1:100){ #repeat for 100 permutated labels
    ...
    Aopt=randomAopt_n50[i,r] #this data frame was made to get figure 6 and SI figure 5
...
ScaleFactor=0.05/max(pp_true[1:10,9]) #pp_true was already earlier made. The 9th column shows how good a small set can be predicted with the remaining 788 samples from the whole population. Scalefactor is needed for the right axis.

dfs%>%pivot_longer(cols=c('5-fold 1CV','25-fold 1CV','5-fold DV','25-fold DV','Bootstrap'),names_to='method',values_to = 'value')%>%ggplot(aes(x=ss,y=value,col=method))+geom_line()+geom_hline(yintercept=0.05,linetype='dashed')+labs(y='p-value',x='small set',col='Method')+scale_x_discrete(limits=c(1:10))+geom_line(data=Xdf[1:10,],mapping=aes(x=ss,y=ACC*scaleFactor,col='Reference'),col='orange',linetype='longdash')+scale_y_continuous(name='p-value',sec.axis = sec_axis(~./scaleFactor,name='Accuracy (%)'))+theme(axis.title.y.left=element_text(color="black"),axis.text.y.left=element_text(color="black"),axis.title.y.right=element_text(color="orange"),axis.text.y.right=element_text(color="orange"))+labs(x='Small set',col='')
```

```{r}
#Chunk to make figure 5b and SI figure 3
...
for (i in 1:25){ #repeat process for 25 sets
  ...
  for (r in 1:100){ #100 random labels
    cr=c_random(c=smallc,ss=r)
    Aopt=randomAopt_n50[i,r] #This data frame is already made when generating figure 6 and SI figure 5. Uncomment this line when n=50 in small sets (figure 6). 
    #Aopt=randomAopt_n100[i,r] #uncomment this line when n=100 (SI figure 5)
...
dfs25[1:25,]%>%pivot_longer(cols=c('5-fold 1CV','Bootstrap'),names_to = 'method',values_to = 'value')%>%ggplot(aes(x=ss,y=value,col=method))+geom_line()+geom_hline(yintercept=0.05,linetype='dashed')+labs(y='p-value',x='small set',col='Method')+scale_x_discrete(limits=c(1:25))+geom_line(data=Xdf[1:25,],mapping=aes(x=ss,y=ACC*scaleFactor,col='Reference'),col='orange',linetype='longdash')+scale_y_continuous(name='p-value',sec.axis = sec_axis(~./scaleFactor,name='Accuracy (%)'))+theme(axis.title.y.left=element_text(color="black"),axis.text.y.left=element_text(color="black"),axis.title.y.right=element_text(color="orange"),axis.text.y.right=element_text(color="orange"))+labs(x='Small set',col='')

```

```{r}
#In this Chunk SI figure 4 is made.
#------
...
```{r}
#in this chunk SI figure 2 is made
#-----
